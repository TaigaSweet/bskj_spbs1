...
public class CategoryController {
    ...
    @RequestMapping(value = "/pageSize.do", method = RequestMethod.POST)
    @ResponseBody
    public ServerSponse<List<Category>> listCategory(HttpSession session, @RequestParam(value = "pageNum", defaultValue = "1") Integer pageNum,
                                                     @RequestParam(value = "pageSize", defaultValue = "5") Integer pageSize) {
        ServerSponse<List<Category>> list = categoryService.listCategory(pageNum, pageSize);
        System.out.println(list.getData().size() + "   " + list.getData());
        return list;
    }

    @RequestMapping(value = "/list.do", method = RequestMethod.POST)
    @ResponseBody
    public ServerSponse<List<Category>> listCategorys(HttpSession session) {
        ServerSponse<List<Category>> list = categoryService.listCategorys();
        session.setAttribute("pageCount", ((list.getData().size() - 1) / 5) + 1);
        session.setAttribute("items", list.getData());
