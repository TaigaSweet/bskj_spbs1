...
public class ProductManagerController {
    ...
    //保存的文件路径/bskj_spbs1/src/main/webapp/uploadfile/img
    ...
    public ServerSponse<String> addPicture(@RequestParam(value = "file", required = false) MultipartFile file, HttpServletRequest request) {
        ...
        String msg = "";//返回存储路径 bskj_spbs1\\src\\main\\webapp\\uploadfile\\img"
        int code = 1;
        System.out.println(file);
        String fileName = file.getOriginalFilename();//获取文件名加后缀
        System.out.println(fileName);
        if (fileName != null && fileName != "") {
            String returnUrl = "..\\bskj_spbs1\\src\\main\\webapp\\uploadfile\\img";//存储路径
            String path = "..\\bskj_spbs1\\src\\main\\webapp\\uploadfile\\img"; //文件存储位置
            String fileF = fileName.substring(fileName.lastIndexOf("."), fileName.length());//文件后缀

            fileName = new Date().getTime() + "_" + new Random().nextInt(10000) + fileF;//新的文件名
            //先判断文件是否存在
            Date date = new Date();
